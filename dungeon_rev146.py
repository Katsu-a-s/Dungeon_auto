...
def modifier_defpill_mult():
    ...
    return 1.5 if floor_modifier == "bastion" else 1.0
...
def roll_shrine_outcome():
    ...
    # 「shrine_gambler」は初回踏破のみで解除されるが、Totemic/Card Shark/
    # Master Trader/Dungeon Wardenなどと同様に、繰り返し祠に足を運び続ける
    # ことを評価する累積目標が無かったため、既存の記録(shrines_used)を
    # そのまま活かせるやり込み目標として追加した。
    if load_stats().get("shrines_used", 0) >= 20:
        unlock_achievement("shrine_devotee")
...
def draw_achievement_toast(bg):
    ...
    どの画面状態でも同じように目立つ。"""
    global achievement_toast_timer, achievement_toast_label, achievement_sound_pending
        ...
        if idx == 0:
            ...
            if tmr == 11:
                ...
                if ((in_boss_battle or in_echo_battle) and not boss_phase2 and emy_life > 0
                    ...
                    # フェーズ2移行の瞬間は既にシェイク・SE・メッセージで演出していたが、
                    # MASSIVE HIT!/コンボフィニッシャー/自己ベストコンボ更新のような
                    # 画面フラッシュが無く、他の「見せ場」演出に比べて地味だった。
                    # 同じdraw_crit_flash()の仕組みを、専用の血のような赤色で流用し、
                    # 「ボスが激怒した瞬間」であることを視覚的にも強調する
                    # (screen_flash_enabled設定にも既存の仕組みのまま追従する)。
                    crit_flash_color = (200, 30, 20)
            ...
            if tmr == 1:
                ...
                se[2].play()
            if tmr == 6:
                # 攻撃側のダメージには既にdamage_popupsで数値がふわっと浮かぶ演出が
                # あったが、回復側(ポーション)には数値のフィードバックが一切無く、
                # 実際どれだけ回復したのか目分量でしか分からなかった。
                # spawn_damage_popup()をそのまま流用し、緑色の"+回復量"ポップアップを
                # プレイヤーの頭上(既存の被ダメージポップアップと同じ座標)に表示する。
                heal_amt = pl_lifemax - pl_life
                pl_life = pl_lifemax
                potion -= 1
                record_stat("potions_used")
                if heal_amt > 0:
                    spawn_damage_popup(190, 585, f"+{heal_amt}", (90, 230, 110))
                if load_stats().get("potions_used", 0) >= 20:
                    unlock_achievement("alchemist")
            if tmr == 11:
                idx = 13
