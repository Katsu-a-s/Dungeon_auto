...
def init_battle():
    ...
    if floor >= 70:
        ...
        # typ30「Chain Warden」だけが混ざるようにした(それ未満の階には出ない)。
        typ = random.randint(8, 30)
        lev = random.randint(floor - 2, floor)
    elif floor >= 30:
        # フロア30〜69(ステージ2以降、typ27-29の深層専用勢が出ない帯)は
        # ずっとtyp8-26の顔ぶれだけが使い回され続けていたため、typ30
        # 「Chain Warden」だけはこの中盤の帯にも混ざるようにした(範囲が
        # 8-26と非連続になるため、randintではなくchoiceでリストを結合する)。
        typ = random.choice(list(range(8, 27)) + [30])
        lev = random.randint(floor - 2, floor)
    elif floor >= 15:
