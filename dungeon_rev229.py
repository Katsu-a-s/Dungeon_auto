...
ACHIEVEMENT_DEFS = [
    ...
    # hero_awakened(初回覚醒)・full_ascension(全20人覚醒)の2段構成は
    # 他の「初回+累積(全種)」実績と同じパターン。rev217でAntiquarianが
    # 20人目として加わったため、判定はlen(CHARACTER_ORDER)を参照しており
    # 表記のみ19→20に更新した。
    ("hero_awakened", "Awaken your first Hero"),
    ("full_ascension", "Awaken all 20 Heroes"),
    ...
    ("grand_transcendence", "Transcend all 20 Heroes"),
...
ACHIEVEMENT_PROGRESS = {
    ...
    "full_ascension": ("characters_awakened_count", 20),
    ...
    "grand_transcendence": ("characters_transcended_count", 20),
...
CHARACTER_ORDER = ["warrior", "guardian", "scholar", "scout", "rogue", "berserker", "prospector", "trader", "monk", "cleric", "pyromancer", "duelist", "reaver", "vagabond", "apothecary", "marshal", "ranger", "vanguard", "virtuoso", "antiquarian", "sentinel", "cartographer"]
HERO_GRID_ROWS = 11  # キャラクター選択画面(idx==49)のグリッド行数。2列×11行=22枠。rev204のMarshal追加で2列×8行(16枠)が埋まりきった後、rev209のRanger追加で2列×9行、rev211のVanguard追加で18/18枠が埋まり、rev215のVirtuoso追加で2列×10行(20枠、19/20)に拡張し、rev217のAntiquarian追加で20/20枠が埋まりきり、rev223のSentinel追加で2列×11行(22枠、21/22)に拡張し、rev224のCartographer追加で22/22枠が埋まりきった(次の増員には行数のさらなる拡張が必要)。
...
def make_dungeon():
    ...
    if floor >= 6 and (modifier_charm_shrine_guaranteed() or random.randint(0, 99) < CHARM_SHRINE_CHANCE):
        place_charm_shrine()
    if floor >= 4 and random.randint(0, 99) < RUNE_ALTAR_CHANCE:
        place_rune_altar()
