...
def flee_chance_pct():
    ...
    秘宝(Featherlight Cloak)のボーナスを加える"""
    return min(95, FLEE_CHANCE_PCT + modifier_flee_bonus() + char_params().get("flee_bonus", 0) + relic_flee_bonus())
...
def apply_pet_bonuses():
    ...
    global pet_bond_achieved_this_run, pet_dmg_reduction_mult
...
CHARACTER_ORDER = ["warrior", "guardian", "scholar", "scout", "rogue", "berserker", "prospector", "trader", "monk", "cleric", "pyromancer", "duelist", "reaver", "vagabond"]
HERO_GRID_ROWS = 7  # キャラクター選択画面(idx==49)のグリッド行数。描画側・矢印キー操作側で共通利用する。
...
def init_battle():
    ...
    if floor > 90:
        ...
        typ = random.choice(list(range(8, 32)) + [32, 33, 36, 38, 39])
        ...
        typ = random.choice(list(range(8, 32)) + [33, 36, 38, 39])
        lev = random.randint(floor - 2, floor)
    elif floor >= 30:
        ...
        # この中盤〜深層の帯から混ぜる。
        typ = random.choice(list(range(8, 27)) + [30, 31, 33, 38, 39])
        lev = random.randint(floor - 2, floor)
    elif floor >= 15:
        ...
        if idx == 0:
            ...
            sections = [
                ("Title Screen", (200, 200, 255), [
                    ...
                    "[V] True Depths (after 3 Hidden Stage clears)",
            ...
            flee_bonus_total = modifier_flee_bonus() + char_params().get("flee_bonus", 0) + relic_flee_bonus()
            ...
            if tmr == 11:
                ...
                # 既存のpl_poison付与ロジック(idx==13)と同じお作法。
                enemy_poison_chance = relic_enemy_poison_chance() + modifier_enemy_poison_chance_bonus()
