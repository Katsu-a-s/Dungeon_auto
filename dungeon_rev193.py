...
CHARACTER_TYPES = {
    ...
    # 2列×HERO_GRID_ROWS(6)行のグリッドがちょうど12/12で埋まる。
...
CHARACTER_ORDER = ["warrior", "guardian", "scholar", "scout", "rogue", "berserker", "prospector", "trader", "monk", "cleric", "pyromancer", "duelist"]
...
def init_battle():
    ...
    if floor > 90:
        ...
        typ = random.choice(list(range(8, 32)) + [32, 33, 36, 38])
        ...
        # ここでも引き続き出現させる。
        typ = random.choice(list(range(8, 32)) + [33, 36, 38])
        lev = random.randint(floor - 2, floor)
    elif floor >= 30:
        ...
        # 新しい駆け引きとしてこの中盤の帯から混ぜる。
        typ = random.choice(list(range(8, 27)) + [30, 31, 33, 38])
        lev = random.randint(floor - 2, floor)
    elif floor >= 15:
