...
CHARACTER_ORDER = ["warrior", "guardian", "scholar", "scout", "rogue"]
...
def battle_command(bg, fnt, key):
    ...
    if key[K_SPACE] or key[K_RETURN]:
        ent = True
    ultimate_ready = combo_count >= ULTIMATE_COMBO_REQUIREMENT
    for i in range(len(COMMAND)):
        ...
        if i == 6:
            if ultimate_ready:
                ...
                draw_text(bg, f"({combo_count}/{ULTIMATE_COMBO_REQUIREMENT} combo)", 190, 200+i*60, fnt, (150, 140, 120))
    return ent
...
def main():
    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == KEYDOWN:
                ...
                if event.key == K_n and idx == 0:
                    ...
                    tmr = 0
                if idx == 49:
                    char_key_map = {K_1: 0, K_2: 1, K_3: 2, K_4: 3, K_5: 4}
        ...
        if idx == 0:
            ...
            draw_text(screen, "[1-5] Choose   [Esc] Back", 260, 560, fontS, WHITE)
            ...
            if idx == 11 and battle_command(screen, font, key):
                if not flg_action:
                    if btl_cmd == 0:
                        ...
                        if combo_count >= ULTIMATE_COMBO_REQUIREMENT:
                            idx = 29
                            tmr = 0
                            flg_action = True
                        else:
                            if not no_ultimate_shown:
                                set_message(f"Ultimate not charged! ({combo_count}/{ULTIMATE_COMBO_REQUIREMENT} combo)", (200, 90, 90))
            ...
            if tmr == 11:
                dmg = int(pl_str * ud["mult"]) + random.randint(0, ud["bonus_rand"])
