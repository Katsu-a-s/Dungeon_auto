...
settings_cursor = 0  # 設定画面でのカーソル位置(0=BGM, 1=SE, 2=Mute All, 3=Screen Shake)
...
def modifier_vision_delta():
    return -2 if floor_modifier == "foggy" else 0
...
def modifier_elite_chance_bonus():
    ...
    return 12 if floor_modifier == "elite_grounds" else 0
...
def load_settings():
    ...
    global bgm_volume, se_volume, muted, screen_shake_enabled
    try:
    ...
    try:
        with open(SETTINGS_FILE, "w") as f:
            json.dump({"bgm_volume": bgm_volume, "se_volume": se_volume, "muted": muted,
                       "screen_shake_enabled": screen_shake_enabled}, f)
    except Exception as e:
...
def put_event():
    ...
    event_pool = [1,1] + [2]*monster_weight + [4]*trap_weight + [14]*trapchest_weight
    if is_blood_moon:
...
def move_player(key):
    ...
    if dungeon[pl_y][pl_x] == 1:
        dungeon[pl_y][pl_x] = 0
        if floor >= 6 and random.randint(0, 99) < MIMIC_CHANCE:
    ...
    if dungeon[pl_y][pl_x] == 2:
        ...
        if r < 45:
            ...
            if treasure == 3: food = food + int(30 * skill_food_yield_mult)
            if treasure == 4: food = food + int(60 * skill_food_yield_mult)
            if treasure == 9: food = food + int(45 * skill_food_yield_mult)
            record_item_seen(treasure)
            idx = 3
            tmr = 0
        else:
    ...
    if dungeon[pl_y][pl_x] == 4:
        dungeon[pl_y][pl_x] = 0
        tdm = diff_params()["trap_dmg_mult"] * skill_trap_dmg_mult
        r = random.randint(0, 99)
        if r < 10:
            base_dmg = 50
        elif r < 30:
...
def draw_battle(bg, fnt):
    ...
    if combo_count >= 2:
        if combo_count >= COMBO_FINISHER_THRESHOLD:
            ...
            draw_text(bg, f"COMBO x{combo_count}", 60, status_y, fnt, (255, 160, 0))
        status_y += 24
    if pet_type is not None:
...
def main():
    ...
    global bgm_volume, se_volume, settings_cursor, muted, screen_shake_enabled
    ...
    while True:
            ...
            if event.type == KEYDOWN:
                ...
                if idx == 56:
                    if event.key == K_ESCAPE:
                        ...
                        settings_cursor = (settings_cursor + delta_row) % 4
                ...
                # 旅の商人(idx==48)での取引
                if idx == 48:
                    if event.key == K_1 and food >= 80:
                        food -= 80
                        potion += 1
                        info_message = "Bought a Potion!"
                        info_timer = 40
                        record_stat("merchant_trades")
                    elif event.key == K_2 and potion >= 2:
                        potion -= 2
                        blazegem += 1
                        info_message = "Traded for a Blaze gem!"
                        info_timer = 40
                        record_stat("merchant_trades")
                    elif event.key == K_3 and blazegem >= 2:
                        blazegem -= 2
                        def_pill += 1
                        info_message = "Traded for a Defense Pill!"
                        info_timer = 40
                        record_stat("merchant_trades")
                    elif event.key == K_4 and pet_type is None and food >= 150:
                        food -= 150
                        hatch_random_pet()
                        info_message = f"{PET_TYPES[pet_type]['name']} hatched!"
                        info_timer = 40
        ...
        if idx == 0:
            ...
            exp_col = (120, 255, 150) if exp_pct >= EXPLORATION_BONUS_THRESHOLD else WHITE
            ...
            if tmr == 5:
                if pending_bonus_room:
                    ...
                    if explore_pct >= EXPLORATION_PERFECT_THRESHOLD:
                        ...
                        if load_stats().get("floors_fully_explored", 0) >= 10:
            ...
            col1 = WHITE if food >= 80 else (110, 110, 110)
            col2 = WHITE if potion >= 2 else (110, 110, 110)
            col3 = WHITE if blazegem >= 2 else (110, 110, 110)
            draw_text(screen, "[1] 80 Food -> 1 Potion", 200, 280, fontS, col1)
            draw_text(screen, "[2] 2 Potion -> 1 Blaze gem", 200, 320, fontS, col2)
            draw_text(screen, "[3] 2 Blaze gem -> 1 Defense Pill", 200, 360, fontS, col3)
            if pet_type is None:
                col4 = WHITE if food >= 150 else (110, 110, 110)
                draw_text(screen, "[4] 150 Food -> Pet Egg", 200, 400, fontS, col4)
            if info_timer > 0 and info_message != "":
