...
def start_hidden_stage_challenge():
    ...
    global food, food_acc, potion, blazegem, pl_poison, pl_bleed, pl_charge, battle_took_damage
...
ACHIEVEMENT_DEFS = [
    ...
    # hero_awakened(初回覚醒)・full_ascension(全16人覚醒)の2段構成は
    # 他の「初回+累積(全種)」実績と同じパターン。
    ("hero_awakened", "Awaken your first Hero"),
    ("full_ascension", "Awaken all 16 Heroes"),
]
...
ACHIEVEMENT_PROGRESS = {
    ...
    "full_ascension": ("characters_awakened_count", 16),
}
...
CHARACTER_ORDER = ["warrior", "guardian", "scholar", "scout", "rogue", "berserker", "prospector", "trader", "monk", "cleric", "pyromancer", "duelist", "reaver", "vagabond", "apothecary", "marshal"]
HERO_GRID_ROWS = 8  # キャラクター選択画面(idx==49)のグリッド行数。2列×8行=16枠。rev201でApothecary追加により7→8に拡張、rev204のMarshal追加でちょうど16/16が埋まった。
...
def init_battle():
    ...
    global pl_charge, pl_poison, pl_bleed, battle_took_damage
    ...
    if floor > 90:
        ...
        # ここでも引き続き出現させる。typ41「Bloodthorn Revenant」(今回追加分)
        # もfloor>=30から混ざる帯としてここでも引き続き出現させる。
        typ = random.choice(list(range(8, 32)) + [32, 33, 36, 38, 39, 40, 41])
        ...
        # Revenant」(今回追加分)もfloor>=30から混ざる帯としてここでも
        # 引き続き出現させる。
        typ = random.choice(list(range(8, 32)) + [33, 36, 38, 39, 40, 41])
        lev = random.randint(floor - 2, floor)
    elif floor >= 30:
        ...
        # 新しく持ち込む敵として、この中盤の帯から混ぜる。
        typ = random.choice(list(range(8, 27)) + [30, 31, 33, 38, 39, 41])
        lev = random.randint(floor - 2, floor)
    elif floor >= 15:
...
def main():
    ...
    global pl_exp_mult, pl_charge, pl_poison, pl_bleed
