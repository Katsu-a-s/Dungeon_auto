...
def start_hidden_stage_challenge():
    ...
    global food, food_acc, potion, blazegem, pl_poison, pl_bleed, pl_frozen, pl_weaken, pl_sunder, pl_charge, battle_took_damage
...
def init_battle():
    ...
    global pl_charge, pl_poison, pl_bleed, pl_frozen, pl_weaken, pl_sunder, battle_took_damage
    ...
    if floor > 90:
        ...
        typ = random.choice(list(range(8, 32)) + [32, 33, 36, 38, 39, 40, 41, 42, 43, 44])
        ...
        # typ44「Rust Golem」(今回追加分)もfloor>=30から混ざる帯として、ここでも引き続き出現させる。
        typ = random.choice(list(range(8, 32)) + [33, 36, 38, 39, 40, 41, 42, 43, 44])
        lev = random.randint(floor - 2, floor)
    elif floor >= 30:
        ...
        typ = random.choice(list(range(8, 27)) + [30, 31, 33, 38, 39, 41, 42, 43, 44])
        lev = random.randint(floor - 2, floor)
    elif floor >= 15:
...
def main():
    ...
    global pl_exp_mult, pl_charge, pl_poison, pl_bleed, pl_frozen, pl_weaken, pl_sunder
    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == KEYDOWN:
                ...
                if event.key == K_ESCAPE and idx == 70:
                    idx = 45
                    tmr = 0
                if idx == 70 and event.key in (K_1, K_2, K_3, K_4, K_5, K_6, K_7, K_8, K_0):
                    ...
                    charm_key_map = {K_1: 0, K_2: 1, K_3: 2, K_4: 3, K_5: 4, K_6: 5, K_7: 6, K_8: 7}
        ...
        if idx == 0:
            ...
            draw_text(screen, "[1-8] Equip  [0] Unequip  [Esc] Back", 60, bottom_y + 30, fontXS, CYAN)
            label = "[Esc] Back"
            draw_button(screen, fontS, 340, 640, fontS.size(label)[0] + 30, 28, label, "back_to_records",
                        base_color=(110, 110, 120), mouse_pos=mouse_pos)
