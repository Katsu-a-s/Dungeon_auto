...
def pet_is_bonded():
    return pet_type is not None and (floor - pet_hatched_floor) >= PET_BOND_FLOOR_THRESHOLD
...
def hatch_random_pet():
    ...
    global pet_type, pet_hatched_floor
...
CHARACTER_ORDER = ["warrior", "guardian", "scholar", "scout", "rogue", "berserker", "prospector", "trader"]
...
# 【UI改善】特殊な戦闘ギミックを持つ深層モンスター(Mirror Wraith/Hollow Widow)は、
# これまでバトル中の一度きりの注意書きでしか仕組みを確認できず、図鑑(Bestiary)で
# 見返しても「ただの敵の一覧」にしか見えなかった。図鑑の詳細画面にも同じ説明を
# 常設表示することで、再戦前に予習したり見た目で仕組みを思い出せるようにした。
...
def init_battle():
    ...
    if floor >= 70:
        ...
        # だけが混ざるようにした(それ未満の階には出ない)。
        typ = random.randint(8, 29)
...
def main():
    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == KEYDOWN:
                ...
                if event.key == K_n and idx == 0:
                    ...
                    tmr = 0
                if idx == 49:
                    char_key_map = {K_1: 0, K_2: 1, K_3: 2, K_4: 3, K_5: 4, K_6: 5, K_7: 6, K_8: 7}
        ...
        if idx == 0:
            ...
            draw_text(screen, "[1-8] Choose   [Esc] Back", 260, 630, fontS, WHITE)
            ...
            if tmr == 5:
                ...
                emy_step = 30
            if tmr == 9:
                counter_def_bonus = int(COUNTER_DEF_BONUS * (modifier_counter_bonus() + skill_counter_bonus))
            ...
            if tmr == 20:
                pl_str_eff = pl_str + pet_str_bonus
                cdmg = int(pl_str_eff * COUNTER_DMG_MULT * (modifier_counter_bonus() + skill_counter_bonus)) + random.randint(0, 10)
            ...
            if pet_type is not None:
                ...
                if pet_is_bonded():
                    ...
                    floors_left = max(0, PET_BOND_FLOOR_THRESHOLD - (floor - pet_hatched_floor))
