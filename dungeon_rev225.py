...
def start_hidden_stage_challenge():
    ...
    global food, food_acc, potion, blazegem, pl_poison, pl_bleed, pl_frozen, pl_weaken, pl_sunder, pl_restrain, pl_burn, pl_charge, battle_took_damage
...
FLOOR_MODIFIERS = {
    ...
    "stalwart": {"name": "Stalwart Floor", "desc": "All status ailment chance (poison/bleed/frozen/weaken/sunder/restrain/burn) -15%pt here", "color": (190, 205, 215)},
...
def modifier_status_resist_bonus():
    """Stalwart Floor(rev223追加)が、毒/出血/凍結/哀弱/錆蝕/拘束/炎上7種すべての
...
ULTIMATE_DEFS = {
    ...
    # (毒/出血/凍結/哀弱/錆蝕/拘束/炎上)をすべて浄化する新しいcleanseキーを
...
def init_battle():
    ...
    global pl_charge, pl_poison, pl_bleed, pl_frozen, pl_weaken, pl_sunder, pl_restrain, pl_burn, battle_took_damage
    ...
    if floor > 90:
        ...
        typ = random.choice(list(range(8, 32)) + [32, 33, 36, 38, 39, 40, 41, 42, 43, 44, 45, 46])
        ...
        # typ45「Grasping Revenant」(今回追加分)もfloor>=30から混ざる帯として、ここでも引き続き出現させる。
        # typ46「Ember Ghast」(今回追加分)もfloor>=30から混ざる帯として、ここでも引き続き出現させる。
        typ = random.choice(list(range(8, 32)) + [33, 36, 38, 39, 40, 41, 42, 43, 44, 45, 46])
        lev = random.randint(floor - 2, floor)
    elif floor >= 30:
        ...
        typ = random.choice(list(range(8, 27)) + [30, 31, 33, 38, 39, 41, 42, 43, 44, 45, 46])
        lev = random.randint(floor - 2, floor)
    elif floor >= 15:
...
def main():
    ...
    global pl_exp_mult, pl_charge, pl_poison, pl_bleed, pl_frozen, pl_weaken, pl_sunder, pl_restrain, pl_burn
    ...
    while True:
        ...
        if idx == 0:
            ...
            if tmr == 1:
                ...
                if ud.get("cleanse") and (pl_poison or pl_bleed or pl_frozen or pl_weaken or pl_sunder or pl_restrain or pl_burn):
                    pl_poison = 0
                    pl_bleed = 0
                    pl_frozen = 0
                    pl_weaken = 0
