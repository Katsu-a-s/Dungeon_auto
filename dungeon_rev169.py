...
chest_open_sound_pending = False   # 宝箱を開けた瞬間の再生待ちフラグ(se[1]を流用、レアアイテムは専用ジングルのまま)
...
def move_player(key):
    ...
    global chest_open_sound_pending
    ...
    if dungeon[pl_y][pl_x] == 1:
        ...
        if treasure not in (5, 6, 7, 8):
            # 指輪/アミュレット等のレア枠(5〜8)は専用ジングルが別にあるので
            # 二重に鳴らさない。それ以外(ポーション/爆炎石/食料半減/ペットの卵等)
            # の宝箱開封には、これまで何の音も無かったのでここで音を付ける。
            chest_open_sound_pending = True
        idx = 3
        tmr = 0
        return
...
def main():
    ...
    global chest_open_sound_pending, hidden_wall_sound_pending
    global _autosave_floor_cache
    ...
    while True:
        ...
        if chest_open_sound_pending:
            se[1].play()
            chest_open_sound_pending = False
