...
CHARACTER_ORDER = ["warrior", "guardian", "scholar", "scout", "rogue", "berserker", "prospector", "trader", "monk", "cleric", "pyromancer", "duelist", "reaver", "vagabond", "apothecary"]
HERO_GRID_ROWS = 8  # キャラクター選択画面(idx==49)のグリッド行数。描画側・矢印キー操作側で共通利用する。rev201でApothecary追加により7→8に拡張。
...
def init_battle():
    ...
    if floor > 90:
        ...
        # ここでも引き続き出現させる。
        typ = random.choice(list(range(8, 32)) + [32, 33, 36, 38, 39, 40])
        ...
        # 深層専用モンスターとしてここで初めて出現させる。
        typ = random.choice(list(range(8, 32)) + [33, 36, 38, 39, 40])
        lev = random.randint(floor - 2, floor)
    elif floor >= 30:
    ...
    while True:
        ...
        if idx == 0:
            ...
            if poison_chance_total > 0:
                ...
                status_y += 26
            # 秘宝Thunderclap Idol・フロア特性Stormbound(rev202追加)も、
            # POISON CHANCEと同じ理由で合計値をダンジョン探索中から確認できるようにした。
            stun_chance_total = relic_enemy_stun_chance() + modifier_enemy_stun_chance_bonus()
            ...
            if tmr == 11:
                ...
                # 伴わない状態異常はこれが初めて)。
                enemy_stun_chance = relic_enemy_stun_chance() + modifier_enemy_stun_chance_bonus()
