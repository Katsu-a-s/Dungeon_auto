...
CHARACTER_ORDER = ["warrior", "guardian", "scholar", "scout", "rogue", "berserker", "prospector", "trader", "monk", "cleric", "pyromancer", "duelist", "reaver", "vagabond", "apothecary", "marshal", "ranger", "vanguard", "virtuoso", "antiquarian", "sentinel"]
HERO_GRID_ROWS = 11  # キャラクター選択画面(idx==49)のグリッド行数。2列×11行=22枠。rev204のMarshal追加で2列×8行(16枠)が埋まりきった後、rev209のRanger追加で2列×9行、rev211のVanguard追加で18/18枠が埋まり、rev215のVirtuoso追加で2列×10行(20枠、19/20)に拡張し、rev217のAntiquarian追加で20/20枠が埋まりきり、rev223のSentinel追加で2列×11行(22枠、21/22)に拡張した。
...
def draw_dungeon(bg, fnt):
    ...
    dp = diff_params()
    reveal_radius(max(1, BASE_VISION_RADIUS + dp["vision_radius_bonus"] + modifier_vision_delta() + skill_vision_bonus + pet_vision_bonus))
...
def main():
    ...
    while True:
        ...
        if idx == 0:
            ...
            # rev219でExecutioner's Coinが加わり17種→18種になった。
            # rev206/208でSerpent's Ward・Slayer's Emblemが加わった際は行間を
            # 詰め直さないまま17種まで増えており、30pxのままだと最後の行の
            # 効果説明文(y=150+16*30+26=656)が[Esc] Back行(y=640〜668)と
            # 実際に重なって読みにくくなっていた。同じ理由で30pxから24pxに
            # さらに詰め、18種でも重ならないようにした。
            ROW_H2 = 24
            ...
            # 今回Charm of Steadfast Willを追加して7種になると、73のままでは
            # 最終行(7行目)の説明文が[Esc] Backボタン(y=640)と再び近づき
            # すぎるため、6種を境にさらに行間を詰めた(6種以下なら73のまま)。
            # 今回Charm of Rustproof Wardを追加して8種になると、64のままでは
            # 最終行の下の合計数表示・[1-7] Equip案内が[Esc] Backボタン
            # (y=640)と重なってしまうため、7種を境にさらに行間を詰めた
            # (7種以下なら64のまま)。行間そのものに加え、説明文(desc)の
            # y+46オフセットも詰めないと行間だけ詰めても次の行の見出しと
            # 説明文が重なってしまうため、8種のときはdesc_yも合わせて詰める。
            # 今回Charm of Unshackled Stepsを追加して9種になると、56のままでは
            # 最終行の下の合計数表示・[1-9] Equip案内が[Esc] Backボタン
            # (y=640)と再び重なってしまうため、8種を境にさらに行間を詰めた
            # (8種以下なら56のまま)。実際にヘッドレスレンダリングで9種
            # 表示時の見た目を確認し、重ならない値(48/38)を選んだ。
            # 今回Charm of Quenching Balmを追加して10種になると、48のままでは
            # 同じ[Esc] Backボタン(y=640)と合計数表示・Equip案内が再び重なって
            # しまうことをヘッドレスレンダリングで確認したため、9種を境に
            # さらに行間を詰めた(9種以下なら48/38のまま)。
            if len(CHARM_DEFS) <= 5:
                ROW_H3 = 85
                desc_y = 46
            elif len(CHARM_DEFS) <= 6:
                ROW_H3 = 73
                desc_y = 46
            elif len(CHARM_DEFS) <= 7:
                ROW_H3 = 64
                desc_y = 46
            elif len(CHARM_DEFS) <= 8:
                ROW_H3 = 56
                desc_y = 38
            elif len(CHARM_DEFS) <= 9:
                ROW_H3 = 48
                desc_y = 38
            else:
                ROW_H3 = 42
            ...
            if tmr == 9:
                ...
                if typ == 41 and pl_bleed == 0 and random.randint(0, 99) < BLOODTHORN_BLEED_CHANCE + modifier_bleed_chance_bonus() - char_params().get("status_resist_bonus", 0) - modifier_status_resist_bonus():
