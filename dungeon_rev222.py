...
CHARACTER_ORDER = ["warrior", "guardian", "scholar", "scout", "rogue", "berserker", "prospector", "trader", "monk", "cleric", "pyromancer", "duelist", "reaver", "vagabond", "apothecary", "marshal", "ranger", "vanguard", "virtuoso", "antiquarian"]
HERO_GRID_ROWS = 10  # キャラクター選択画面(idx==49)のグリッド行数。2列×10行=20枠。rev204のMarshal追加で2列×8行(16枠)が埋まりきった後、rev209のRanger追加で2列×9行、rev211のVanguard追加で18/18枠が埋まり、rev215のVirtuoso追加で2列×10行(20枠、19/20)に拡張し、rev217のAntiquarian追加でちょうど20/20枠が埋まりきった。
    ...
    while True:
        ...
        if idx == 0:
            ...
            if tmr == 9:
                ...
                if typ == 43 and pl_weaken == 0 and random.randint(0, 99) < WAILING_BANSHEE_WEAKEN_CHANCE + modifier_weaken_chance_bonus() - charm_weaken_resist_bonus():
                    pl_weaken = WAILING_BANSHEE_WEAKEN_STACKS
                    set_message("Weakened!", (150, 120, 190))
                if typ == 44 and pl_sunder == 0 and random.randint(0, 99) < RUST_GOLEM_SUNDER_CHANCE + modifier_sunder_chance_bonus() - charm_sunder_resist_bonus():
                    pl_sunder = RUST_GOLEM_SUNDER_STACKS
                    set_message("Armor sundered!", (170, 95, 40))
                if typ == 45 and pl_restrain == 0 and random.randint(0, 99) < GRASPING_REVENANT_RESTRAIN_CHANCE + modifier_restrain_chance_bonus() - charm_restrain_resist_bonus():
                    pl_restrain = GRASPING_REVENANT_RESTRAIN_TURNS
                    set_message("Restrained!", (120, 100, 150))
                if typ == 46 and pl_burn == 0 and random.randint(0, 99) < EMBER_GHAST_BURN_CHANCE + modifier_burn_chance_bonus() - charm_burn_resist_bonus():
